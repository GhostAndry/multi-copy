...
int delta_copy_file(const char *src, const char *dest) {
    ...
    if (stat(src, &stat_src) != 0) return -1;
    if (stat(dest, &stat_dest) != 0) return copy_file(src, dest);
        ...
        if (stat(dest_path, &st) == 0) {
            ...
            if (compute_md5(dest_path, md5_dest) != 0) {
                ...
                if (memcmp(md5_src, md5_dest, MD5_DIGEST_LENGTH) != 0) {
                    need_copy = 1;
                    pthread_mutex_lock(&print_mutex);
                    wprintw(topWin, "[Thread %d] File modificato, aggiorno: %s\n", 
                            thread_id, relative_filename);
                    wrefresh(topWin);
                    pthread_mutex_unlock(&print_mutex);
                } else {
                    pthread_mutex_lock(&print_mutex);
                    wprintw(topWin, "[Thread %d] File invariato: %s\n", thread_id, relative_filename);
                    wrefresh(topWin);
                    pthread_mutex_unlock(&print_mutex);
                }
            }
            ...
            need_copy = 1;
            pthread_mutex_lock(&print_mutex);
            wprintw(topWin, "[Thread %d] Nuovo file, copio: %s\n", thread_id, relative_filename);
            wrefresh(topWin);
            pthread_mutex_unlock(&print_mutex);
        }
        ...
        if (need_copy) {
            ...
            if (stat(dest_path, &st) == 0) {
                if (delta_copy_file(src_path, dest_path) == 0) {
                    pthread_mutex_lock(&print_mutex);
                    wprintw(topWin, "[Thread %d] Delta-copy completata per: %s\n", 
                            thread_id, relative_filename);
                    wrefresh(topWin);
                    pthread_mutex_unlock(&print_mutex);
                } else {
                ...
                copy_file(src_path, dest_path);
                pthread_mutex_lock(&print_mutex);
                wprintw(topWin, "[Thread %d] Copiato (nuovo): %s\n", 
                        thread_id, relative_filename);
                wrefresh(topWin);
                pthread_mutex_unlock(&print_mutex);
            }
